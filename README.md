...
- 複数の子どもに対応(タブ切り替え)
- 昨日・今日・明日の天気をアクセスのたび自動取得 → 明日の最高気温がしきい値(既定 23°C)超で半袖/以下で長袖を警告
...
`src/lib/weather.ts` で **Open-Meteo** を直接取得する(無料・APIキー不要・CORS 許可済み・
サーバー関数不要)。
...
- `past_days=1` + `forecast_days=2` で **昨日・今日・明日** の3日分を取得し、天気カードに表示。
- 地域名(都道府県)→ 県庁所在地の緯度経度(全47)を `resolveCoords` で解決。
  「東京都新宿区」のように都道府県名を含む文字列にも対応(完全名照合で 京都/東京 の誤マッチを回避)。
- WMO weather code を日本語ラベル(`wmoToLabel`)に変換し、アイコン判定(`iconFromLabel`)に利用。
