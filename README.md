...
- 複数の子どもに対応(タブ切り替え)
- 明日の天気を気象庁から自動取得(手動入力も可)→ しきい値(既定 23°C)で長袖/半袖を警告
...
`src/lib/weather.ts` で **気象庁オープンデータ**を直接取得する
(`https://www.jma.go.jp/bosai/forecast/data/forecast/{officeCode}.json`、CORS 許可済み・
サーバー関数不要)。

- 地域名(都道府県)→ 気象庁 office コード(全47)を `resolveAreaCode` で解決。
  「東京都新宿区」のように都道府県名を含む文字列にも対応(完全名照合で 京都/東京 の
  誤マッチを回避)。
- 日付は JST で計算(`jstTomorrowDate`)。気温は**週間予報**(`data[1].timeSeries[1]`
  の `tempsMax/Min`)を優先し、空文字 `""` を `0` と誤認しないようガード。取得不可なら
  短期予報の気温にフォールバック。天気テキストは日本語のまま保持しアイコン判定に利用。
- 天気カードの「自動取得」ボタンで取得。失敗時はエラー表示 + 手動入力にフォールバック。

### 今後の拡張(v2 メモ)

- 海外都市対応として Open-Meteo(緯度経度ベース)をフォールバックに追加。
- 都道府県内の複数観測点の選択(現在は代表点 `areas[0]`)。
